...
def set_actor_property(
    ...
    Parameters:
    - actor_name: The name of the actor in the level
    - property_name: The name of the property to set. Can use dot notation for nested properties
                     (e.g., "Settings.ColorSaturation" for PostProcessVolume)
    - property_value: The value to set. Type depends on the property:
        - For float/int: Use a number
        - For bool: Use true/false
        - For Color: Use {"R": 255, "G": 255, "B": 255, "A": 255} or {"R": 1.0, "G": 1.0, "B": 1.0, "A": 1.0}
        - For Vector: Use {"X": 0, "Y": 0, "Z": 0}
        - For Vector4: Use {"X": 0, "Y": 0, "Z": 0, "W": 1}
        - For string: Use a string
    - component_name: Optional. The name of the component to modify
    ...
    Examples:
        # Set light intensity
        set_actor_property("DirectionalLight_1", "Intensity", 3.0)

        # Set light color
        set_actor_property("DirectionalLight_1", "LightColor", {"R": 255, "G": 197, "B": 143, "A": 255})

        # Set PostProcess setting
        set_actor_property("PostProcessVolume_World", "Settings.AutoExposureBias", 0.5)
    """
    unreal = get_unreal_connection()
    if not unreal:
        return {"success": False, "message": "Failed to connect to Unreal Engine"}
